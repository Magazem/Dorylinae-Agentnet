...
func TestDebugNoteKindOnlyWithEnv(t *testing.T) {
	build := func() map[string]mail.Kind {
		rcv, _ := newMailReceiver(nil, nil, nil, make([]byte, ed25519.PublicKeySize), nil, nil, nil, nil, nil, nil)
		return rcv.Kinds
	}
...
func newMNode(t *testing.T, clk *testClock) *mnode {
	...
	n.rcv, n.pusher = newMailReceiver(st.DB(), alog, ks, pub, keys, nil, nil, nil, nil, nil)
