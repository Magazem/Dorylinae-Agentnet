...
// card details and keeps the original paired_at.
...
func (s *Store) List(ctx context.Context) ([]Peer, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT public_key, name, harness, skills, paired_at FROM peers ORDER BY paired_at, public_key`)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.PublicKey, &p.Name, &p.Harness, &skills, &p.PairedAt); err != nil {
