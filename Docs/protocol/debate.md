...
the position against the [schema](#position) and recomputes the commitment from the
request it stored at receipt. On a match it stores A's position as slot 0.
...
- *Changed:* a reveal whose hash differs from the commitment is refused. B's daemon marks
  ...
  `ws.cancel` to A. A correct A daemon never sends one: the reveal is built from the stored
  position and nonce, not by the agent.
...
- **Text.** Line(500) (a single line, no control characters). The approval summary shows it
  through `notify.Clean`, including the review-40 bidi and zero-width rules.
- **Id.** `c-` + 32 hex from `crypto/rand`, chosen at `--constrain`.
...
- **Ordering.** Constraints are not slots. They are a set, ordered in the Decision by
  `(at, id)`. A's `debate.close` lists the ids A holds; both Decisions contain exactly those
  ([decision.md](decision.md#derivation)). A constraint that reaches A after the close is
  not in the record (audited `debate.ignored {reason: "closed"}` on A); B's view marks it
  `late`.
- **Limits.** At most 10 constraints per debate (both sides together, counted on each side
  when adding; the receiver ignores the 11th, `debate.ignored {reason: "limit"}`).
...
- **What the record proves.** A constraint signed into the Decision proves that the named
  ...
  applies), and the other side cannot verify the approval, only the author key.
...
- `grant_create` of a **sensitive** grant to a peer with whom the grantor has a debate in
  `positions`, `rounds` or `converge` is refused with **`debate_open`**.
...
-- recreate requests_idem, requests_state, requests_peer_time exactly as in migrations 11/12

-- approvals: rebuild to add 'debate_constraint' to the kind CHECK (same pattern, same index).
...
CREATE TABLE debate_constraints (
    ...
    at       TEXT NOT NULL,
    state    TEXT NOT NULL CHECK (state IN ('pending_approval', 'active', 'late')),
    approval TEXT,
...
- **Forged human decisions.** Constraints need a local approval (OD-P3-3); an agent using
  AgentNet's interface cannot add one.
