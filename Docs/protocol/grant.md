...
| `fetch_start` | `{"grant", "op", "path"?, "offset"?, "length"?, "cursor"?, "timeout_s"?}` | Holder only. Verifies locally (steps 1–8; a grant that fails is an IPC error carrying the reason, `expired`, `revoked`, `session_not_open`…, and sends nothing), starts the fetch and waits at most 1 s: `{"fetch_id", "state": "pending"\|"complete"\|"failed", "result"?, "error"?: {"code", "message"}}`. `timeout_s` (default 30, 1–300) is how long the fetch may stay pending before it fails with `timeout`. A third fetch to the same grantor in flight is refused locally with `rate_limited` |
| `fetch_status` | `{"fetch_id"}` | Same shape; waits at most 1 s while pending. `result` of a completed `read`: `{"data": "<base64>", "offset", "size", "commit"?}` (`commit` for `git.read` only); of `stat`/`list`: the checked response members only (`entry`, or `entries` and `cursor`?; `commit`?), a malformed response failing with `io_error`. Completed fetches are kept 60 s in memory (then `not_found`), at most 64 (the oldest is dropped first); the 64-fetch limit counts pending fetches only |
...
Pages [../cli/grant.md](../cli/grant.md) and [../cli/fetch.md](../cli/fetch.md) (ticket 2.3c).
