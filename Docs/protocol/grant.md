...
| `fetch_status` | `{"fetch_id"}` | Same shape; waits at most 1 s while pending. `result` of a completed `read`: `{"data": "<base64>", "offset", "size", "commit"?}` (`commit` for `git.read` only); of `stat`/`list`: the response members. Completed fetches are kept 60 s in memory (then `not_found`) |
