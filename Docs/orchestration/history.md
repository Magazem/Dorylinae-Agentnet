# Orchestration history (archived)

The full HANDOFF.md as it stood at the end of Phase 1 (2026-09-23), kept for reference.
The live file is `Docs/orchestration/HANDOFF.md`.
...
| D13 | Own-device helper (owner idea, APPROVED 2026-09-22): separate `device` trust, never created by team/roster/pairing; dedicated link flow confirmed on BOTH devices; only the obeying device can make itself a helper and it holds the scope locally (request types, repos/paths, commands, expiry); off by default, audited, revocable from either side; out-of-scope requests fall back to the normal inbox; one-way hierarchy. Phase 2 feature (with grants). | HANDOFF §3 |
| D14 | Add an optional, size-capped **result payload to `request.complete`** in **1.6a** (Phase 1), so a helper or teammate can return output (test logs, pass/fail, artifacts). Spec change to request.md before 1.6a is dispatched. | request.md (pending) |
...
**D15 (2026-09-22): consumer onboarding** is Phase 4 acceptance item **4.9** in the plan (3 steps, clean machine, 3 OSes, agent-runnable, no config files). No early `setup` wrapper; do not rush it.
...
3. **Phase 1 in flight** (specs merged `b6e824b`, D11). Ticket plan: `Docs/review/11-phase1-tickets.md`. Branch names `p1/<x>`, worktrees under `AgentNet-wt/`.
   ...
   | ~~1.H headless harness~~ | merged `fcbf2d8` + `2eb18a1`. All-Claude round 3/3 both roles (root cause of refusals: Windows shell tool is PowerShell, not Bash; connectors leaked; fixed + snippet clarified). Codex blocked (account limit until 2026-10-02). **Owner offered `agy` (Antigravity CLI v1.2.9, has -p print mode + --dangerously-skip-permissions) as the second harness** — run after the Phase 1 smoke script (avoid concurrent relays). | — | — |
   ...
   | ~~Phase 1 one-machine smoke~~ | merged `989f745` (tests/phase1-smoke.ps1/.sh, ~65 steps, ~8 s, no admin). phase1-manual.md top lists the two-machine-only checks. | — | — |
   | Bug: webhook secret keychain account unscoped ("webhook") → daemons on one machine overwrite each other | T-WebhookKey (also audits other keystore uses) | `wh-keystore` | no |
   | 1.H second harness via agy | T-HarnessAgy | `h-agy` | no |
...
- **CI cost:** the full matrix (3 OS + race + build + cross) ran on every push incl. docs-only commits → ~$0.30/push, - **Watch CI after EVERY push**8 in a day, limit hit. Batch pushes (one per merged ticket, not per handoff note). Proposed CI diet (awaiting owner OK): paths-ignore docs, concurrency cancel, Linux-only on push, full matrix nightly/manual/pre-tag, job timeouts.
