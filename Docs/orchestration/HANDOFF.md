...
3. **Phase 1 in flight** (specs merged `b6e824b`, D11). Ticket plan: `Docs/review/11-phase1-tickets.md`. Branch names `p1/<x>`, worktrees under `AgentNet-wt/`.
   ...
   | 1.H headless agent harness (Claude Code + Codex) | scripts committed `9935911` on `p1/t1-H` (not merged yet). Claude Code sender leg PASSED; **Codex blocked by account usage limit until 2026-10-02** (owner to pick: wait, or name a substitute harness). All-Claude round: infra 100% reliable, but claude -p refused in 1 turn (looked for an "AgentNet tool", not the CLI; owner connectors leaked in). Fix in progress: clearer snippet (agentnet is a CLI run via Bash) + isolate MCP/connectors in the harness, rerun ×3. | `t1-H` | no |
