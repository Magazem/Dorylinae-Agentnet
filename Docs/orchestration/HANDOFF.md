...
Last updated: 2026-09-25 (Friday) evening, home PC: Phases 0-3 merged and tagged, main green. The orchestration rules for this PC: relay all hands-on work to workers (no self-edits, no subagents), poll CI in the background, and always check a worker's worktree/report against the logs before merging.
...
| D36 | 2026-09-26, Phase 4 (partial; the rest of OD-P4-1..21 awaits the owner reading review 49/50): **OD-P4-8 = opt-in** daemon telemetry (asked at setup, non-interactive needs --telemetry on|off; relay counters stay on); **OD-P4-12 = (c) ship wave 1 UNSIGNED** on Windows with a documented SmartScreen click-through (the spec warns unsigned installers teach click-through; revisit before wave 2, signing lead time is 4-6 weeks). |
