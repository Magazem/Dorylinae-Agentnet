...
| 4.9 Simple setup (owner, 2026-09-22) | Consumer onboarding is simple and direct: one command installs the CLI; one `agentnet setup` run logs in, installs and starts the service, uses the hosted relay by default and runs `doctor`; one command connects to a peer or team (`team join <code>`). Every step works non-interactively too, so an agent can run the whole setup from inside its harness (with the human confirming login in the browser) | On a clean macOS, Linux and Windows machine, a new user (or their agent, given only the quickstart) goes from nothing to a first request with those three steps and no hand-edited config files |
